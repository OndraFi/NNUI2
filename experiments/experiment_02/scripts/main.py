...
for i, hidden_size in enumerate(hidden_sizes):
    print(f"\nTraining model with {hidden_size} hidden neurons...")
    nn = neuron_network(input_size, hidden_size, output_size, learning_rate)
    loss_history = nn.train(X, y, epochs)

    # Uložení modelu a chyby
    nn.save_model(f"model_hidden_{hidden_size}.npz")
    np.save(f"loss_hidden_{hidden_size}.npy", loss_history)
...
# 4. Vyhodnocení nejlepšího modelu
final_losses = [loss[-1] for loss in loss_histories]
best_index = np.argmin(final_losses)
best_model = models[best_index]
print(f"\nNejlepší model: {hidden_sizes[best_index]} neuronů, MSE: {final_losses[best_index]}")

# 5. Výstup nejlepšího modelu
